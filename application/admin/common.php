...
/**
 * 将数据库存储的最小库存根据各个单位进制进行换算 返回字符串
 * @param array $unitInfo 商品规格信息数组
 * @param int $stock 最小单位库存数量
 * @return string 1箱2包1盒┃102包1盒┃10201盒
 */
function stockToString(array $unitInfo, int $stock)
{
    $stockString = '';
    $minUnitKey = count($unitInfo['unit_num']) - 1;
    $unitCountNum = count($unitInfo['unit_name']);
    foreach ($unitInfo['unit_name'] as $key => $value){
        $temp = $stock;
        for ($i = $key; $i < $unitCountNum; $i++){
            $calculated = $temp / ($unitInfo['unit_num'][$minUnitKey] / $unitInfo['unit_num'][$i]);
            $stockString .= (int)$calculated . $unitInfo['unit_name'][$i];
            $temp = $temp % ($unitInfo['unit_num'][$minUnitKey] / $unitInfo['unit_num'][$i]);
        }
        if ($key < $minUnitKey){
            $stockString .= '┃';
        }
...
/**
 * 将填写的对应单位的数量 转换成最小单位的数量
 * @param array $inputNumArray 输入的各个单位的数量数组
 * @param array $unitNumArray 各个单位对应的转换进制数
 * @return float|int
 */
function inputStockInfoToMinStock(array $inputNumArray, array $unitNumArray)
{
    $num = 0;
    foreach ($inputNumArray as $k => $inputNum){
        $num += ($inputNum * ($unitNumArray[count($unitNumArray) - 1] / $unitNumArray[$k]));
    }

    return $num;
}

function inputNumToString(array $unitNameArray, array $inputNumArray)
{
    $inputNumString = '';
    foreach ($unitNameArray as $key => $value){
        $inputNumString .= $inputNumArray[$key] . $value;
    }
