...
class Backend extends Controller
{
    ...
    protected function selectpage()
    {
        ...
        if ($primaryvalue !== null) {
            ...
            $where = function ($query) use ($word, $andor, $field, $searchfield, $custom, $join) {
                $logic = $andor == 'AND' ? '&' : '|';
                $searchfield = is_array($searchfield) ? implode($logic, $searchfield) : $searchfield;
                $searchfield = str_replace(',', $logic, $searchfield);
                if (strpos($this->controllerName, '.')){
                    $searchfield = 'fa_' . substr($this->controllerName, strpos($this->controllerName, '.') + 1) . '.' . $searchfield;
                }else{
                    $searchfield = 'fa_' . $this->controllerName . '.' . $searchfield;
                }
                ...
                if ($join['type'] == 2){
                    ...
                    $query->whereIn('fa_goods.id', $goodsIds);
                }
            };
        }
        $adminIds = $this->getDataLimitAdminIds();
        ...
        if ($total > 0) {
            ...
            if (in_array('category_id', $fields)){
                $fields[] = 'category';
                if (!empty($join['merchant_id'])){
                    //存在商户id 则 预载入商户商品售价关系表
                    $with = [
                        'category',
                        'goods_merchant' => function ($query) use ($join) {
                            if (!empty($join['merchant_id'])) {
            ...
            foreach ($datalist as $index => $item) {
                ...
                if (isset($result['unit_info'])){
                    $result['unit_info'] = json_decode($result['unit_info']);
                }

                if (isset($result['category_id'])){
                    $result['category_id'] = $result['category']['name'];
